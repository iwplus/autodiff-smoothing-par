...
@jit
def compute_natural_grad(log_knots, x, y):
    gr = gcv_grad_fn(log_knots, x, y)
    m1, m2 = gr.shape 
    gr_ravel = jnp.ravel(gr)
    H_approx = jnp.outer(gr_ravel, gr_ravel)
    
    solution = jnp.linalg.lstsq(H_approx, gr_ravel, rcond=None)
    gr_nat = jnp.reshape(solution[0], (m1,m2))
    
    return gr_nat, gr

batch_gcv = jit(vmap(lambda eta, knots, g, x, y: gcv(knots - eta * g, x, y), in_axes=(0, None, None, None, None)))
...
for i in range(iter_max):
    gr_nat, raw_grad = compute_natural_grad(knots, x_data, y_data)
    scaler = 1.0 / (jnp.linalg.norm(gr_nat))
    direction = gr_nat * scaler
    
    # Line Search
    losses = batch_gcv(steps, knots, direction, x_data, y_data)
    best_step = steps[jnp.argmin(losses)]
    
    knots = knots - best_step * direction
    current_gcv = jnp.min(losses)
    gcv_history.append(current_gcv)
    
    if i % 500 == 0:
        print(f"Iter {i}: GCV={current_gcv:.6f}, step : {best_step}")
...
print(f"Final knots : {knots_g}")
print(f"Final MSE: {mse}")
print(f"Final GCV: {gcv}")

# np.savetxt("knots_ng.csv", knots, delimiter=",", comments='')
np.savetxt("ng_knots.csv", np.asarray(knots), delimiter=",")
np.savetxt("GCV_ng_spline.csv", gcv_history, delimiter=",", comments='')
